function notifyEditPageTemplate(result){
    ...
    De quais listas você gostaria de receber notificações?<br> 
    ...
    <strong>Com que frequência você gostaria de receber notificações?</strong><br>
    ...
    <br><br>
    <strong id="email_url_label">Endereço de Email:</strong><br><input placeholder="email@domain.com" type=text class='form-control' style="width:100%" name="endpoint" value="${(result.notify.endpoint.length == 0) ? "" : result.notify.endpoint}"><br><br>
    <a href="#" id="advancedNotifyLink" class="link">Mudar o formato do email ou alterar para chamadas em outros aplicativos (avançado)</a><br>
    <div id="advancedNotify" class="subhead sm left">
        <strong>Notification Type:</strong><br>
        Nós podemos enviar notificações por email ou chamar seu App favorito usando IFTTT ou Zapier<br>
        <select class="form-control" name="type" style="width:100%">
            <option value='email' ${(result.notify.type == "email") ? "selected": ""}>Email</option>
            <option value='postback' ${(result.notify.type == "postback") ? "selected" : ""}>Customizar notificação (via Slack/JIRA/Zapier/IFTTT)</option>
        </select><br><br>
        
        <strong>Conteúdo da Notificação:</strong><br>
        <select class="form-control" name="format" style="width:100%">
            ...
            <option value='textsummary' ${(result.notify.format == "textsummary") ? "selected" : ""}>Texto Simples (para notificações via Slack/JIRA) </option>
            <option value='jsontextsummary' ${(result.notify.format == "jsontextsummary") ? "selected": ""}>Texto Simples - JSON Object (para notificações via Slack/JIRA) </option>
            <option value='jsonsummary' ${(result.notify.format == "jsonsummary") ? "selected" : ""}>Resumo Data JSON Object (gestão customizada)</option>
